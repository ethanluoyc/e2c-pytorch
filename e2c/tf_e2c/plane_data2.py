...
class PlaneData(DataSet):
    ...
    def is_colliding(self, p):
        ...
        return np.mean(
            self.im[p[0] - rw:p[0] + rw + 1, p[1] - rw:p[1] + rw + 1]) > 0.05
    ...
    def getXp(self, p):
        ...
        x = np.copy(self.im)
        x[p[0] - rw:p[0] + rw + 1, p[1] - rw:
          p[1] + rw + 1] = 1.  # robot is white on black background
        return x.flat
...
if __name__ == "__main__":
    ...
    if True:
        ...
        x0, u0, x1 = p.sample(2)
        m1 = aa[0].matshow(
            x0[0, :].reshape(w, w), cmap=plt.cm.gray, vmin=0., vmax=1.)
        aa[0].set_title('x(t)')
        m2 = aa[1].matshow(
            x1[0, :].reshape(w, w), cmap=plt.cm.gray, vmin=0., vmax=1.)
        ...
        anim = animation.FuncAnimation(
            fig, updatemat2, frames=100, interval=1000, blit=True, repeat=True)
    ...
    if True:
        ...
        X = p.getXTraj(0)
        mat = ax.matshow(
            X[0, :].reshape((A, B)), cmap=plt.cm.gray, vmin=0., vmax=1.)
        ...
        anim = animation.FuncAnimation(
            fig, updatemat, frames=T - 1, interval=30, blit=True, repeat=True)
        plt.show()
