...
def show_recons_samples(sess, ckptfile):
    ...
    xr, xp = sess.run(
        [e2c.x_recons, e2c.x_predict],
        feed_dict={e2c.x: x_val,
                   e2c.u: u_val,
                   e2c.x_next: x_next_val})
    ...
    def getimgs(x, xnext):
        ...
        for i in range(10):
            startr = i * ph + padsize
            img[startr:startr + B, padsize:padsize + A] = x[i, :].reshape((A,
                                                                           B))
        for i in range(10):
            startr = i * ph + padsize
            img[startr:startr + B, pw + padsize:pw + padsize + A] = xnext[
                i, :].reshape((A, B))
        return img

    fig, arr = plt.subplots(1, 2)
    arr[0].matshow(
        getimgs(x_val, x_next_val), cmap=plt.cm.gray, vmin=0, vmax=1)
...
def viz_z(sess, ckptfile):
    ...
    xy[:,
       1] = 20 - Ps[:,
                    0]  # for the purpose of computing theta, map centered @ origin
    ...
    for i in range(Ps.shape[0] // batch_size):
        ...
        Zs[i * batch_size:(i + 1) * batch_size, :] = sess.run(
            e2c.z, {e2c.x: x_val})
